...
def afspraak():
    ...
    if fouten:
        ...
        return render_template(
            "auto_onderhoud.html",
            active_page="auto_onderhoud",
            form_data={"naam": naam, "email": email, "telefoon": telefoon, "bericht": bericht},
        ), 400

    db = get_db()
    db.execute(
        "INSERT INTO afspraken (naam, email, telefoon, bericht, datum_aangemaakt) VALUES (?, ?, ?, ?, ?)",
        (naam, email, telefoon, bericht, datetime.now().strftime("%Y-%m-%d %H:%M:%S")),
    )
    db.commit()
    ...
    flash("Bedankt! Uw afspraak is verzonden. We nemen snel contact met u op.", "success")
    return redirect(url_for("auto_onderhoud"))
